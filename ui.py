...
def main() -> None:
    ...
    global U_target, qc, error, history
    if "target" not in st.session_state:
        U_target = np.eye(2, dtype=complex) # Noklusējuma unitārā matrica
        st.session_state.target = U_target
    else:
        U_target = st.session_state.target
...
def launch_rotation_decomposition() -> None:
    global U_target, qc, error
    qc, phase, error = qd.rotation_decomposition(U_target)
...
def launch_solovay_kitaev_decomposition(recursion_depth: int, max_length: int) -> None:
    ...
    with st.status("Soloveja-Kitaeva dekompozīcija...") as status:
        ...
        _, phase = qd.remove_global_phase(U_target)
        qc, error, history = qd.solovay_kitaev_decomposition(U_target, recursion_depth, short_circuits, status)
        qc.global_phase = phase
...
def load_results() -> None:
    if history:
        ...
        qc = history[idx]
        error = qd.compare_su2(U_target, Operator(qc).data)
    else:
        qc = st.session_state.qc
        error = st.session_state.error
    ...
    m_col4.metric("Kļūda", f"{error:.2e}")
    with st.expander("Skatīt unitāro matricu"):
        st.write(Operator(qc).data)
