...
from modules.utils import solve_unitary_conjugate, extract_axis_angle, rotation_matrix, remove_global_phase, compare_su2
...
def balanced_group_commutator(u_target: np.ndarray) -> tuple[np.ndarray, np.ndarray]:
    ...
    if u.ndim == 2:
        ...
        logging.info(f"Balanced group commutator computed with error {compare_su2(u_target, v_sol @ w_sol @ v_sol.conj().T @ w_sol.conj().T)}")
        return v_sol, w_sol
